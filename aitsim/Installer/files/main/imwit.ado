...
#delimit ;
capture program drop imwit;
program define imwit, sortpreserve;
version 9.2;
syntax varlist(min=1 max=1)[ ,   ITSCH(string) FTSCH(string) HSIZE(varname)];
...
if `i'== 2 local base`i'_`h'=  - (`r'[`j']  -       0)*(`t2_`h''[`j']-`t1_`h''[`j']);
if `i'!= 2 local base`i'_`h'=  - (`r'[`j']-`r'[`j'-1])*(`t2_`h''[`j']-`t1_`h''[`j']);
...
tempvar wvar ;
qui gen `wvar' = 0 ;
qui replace `wvar' = -`binc'*(`t2_1'[1]-`t1_1'[1])                                                         if `class'==1; 
if `nn1' > 1 {;
forvalues i = 2/`nn1' {;
local j=`i'-1;
 qui replace `wvar' = `basev`i'_`i'' - (`binc'-`se`j'_`i'')*(`t2_`i''[`i']-`t1_`i''[`i'])                             if (`class'==`i') ;
};
};
if `nn1'>=1  qui replace  `wvar' = `basev`nn'_`nn'' - (`binc'-`se`nn1'_`nn'')*(`t2_`nn''[`nn']-`t1_`nn''[`nn'])           if (`class' == `nn' )  ;

if `bun1'==2 qui replace `wvar' = `wvar'/ `hsize' ;



cap drop __imwit;
qui gen  __imwit=`wvar';
end;
