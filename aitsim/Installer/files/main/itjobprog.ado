...
#delimit ;
capture program drop basicprog;
program define basicprog, rclass;
syntax varlist(min=2 max=2) [, HHID(varname) HSize(varname) rank(varname)];
preserve; 
tokenize `varlist';
tempvar fw;
...
capture program drop itjobprog;
program define itjobprog, eclass;
version 9.2;
...
basicprog `pcexp' `1',  hhid(`hhid')  hsize(`_ths') rank(`pcexp');
