...
local itjobs 11 21 22 23 24 25 31 32 33 34 35 41 42 43 44 45 46 47 48 49 ;
foreach number of local  itjobs {;
local _tab_`number' = 0;
};
...
foreach number of local  tjobs {;
local _tab_`number' = 1;
};
/*
foreach number of local  itjobs {;
dis  "_tab_`number' = " `_tab_`number'' ;
};
*/
...
di _n as text in white "{col 5}***************************General information*************************";
	  ...
	  local tnote2   `" " - Enquête:  $S_FN  " , "'   ;
		...
		*/
		...
		if `nscen'==3  qui gen `fpri3' = . ;
				 ...
				 local nps2=0;
	   forvalues i=1/`nitems' {;
	    if `oinf' == 2 local ps`i' = 2;
	    if `ps`i''==1 local nps1= `nps1'+ 1 ;
	    if `ps`i''==2 local nps2= `nps2'+ 1 ;
		};
	  ...
	  if "`ps`i''"=="" local ps`i'=1;	 
      if `ps`i''==1 & `oinf' == 1 .`table'.row `slitems'[`i'] `ip`i''      `su`i''    `fp1`i'' `fp2`i'' `fp3`i''  ; 
	  if `ps`i''==2 & `oinf' == 1 .`table'.row `slitems'[`i'] "i_psch`i'"  "---"   "f_psch`i'"   ; 
	  if `oinf' == 2 local ipsh1_`i' = ""+`ipsch'[`i'];
	    forvalues s = 1/`nscen' {;
	  if `oinf' == 2 local fpsh`s'_`i' = ""+`fpsch`s''[`i'];
	  };
	  if `oinf' == 2 & `nscen'==1 .`table'.row `slitems'[`i'] "`ipsh1_`i''"  "---"   "`fpsh1_`i''"   ; 
	  if `oinf' == 2 & `nscen'==2 .`table'.row `slitems'[`i'] "`ipsh1_`i''"  "---"   "`fpsh1_`i''"  "`fpsh2_`i''"  ; 
	  if `oinf' == 2 & `nscen'==3 .`table'.row `slitems'[`i'] "`ipsh1_`i''"  "---"   "`fpsh1_`i''"  "`fpsh2_`i''"  "`fpsh3_`i''"  ; 
	   *local temn=__compna[`i'];
	   local temn=`slitems'[`i'];
	   local tpsi="";
	   if `ps`i''==2 local tpsi="(non linear)";
       local  rnama `"`rnama' "`temn' `tpsi'""';
	  
	   };
	 
	   .`table'.sep, bot; 
	     tempname ma;
		 mkmat `ipr' `sub' `fpri1'  in 1/`nitems' , matrix(`ma') ;


		 forvalues j=1/`nitems' {;
		  if `oinf' == 2 local ps`j' = 2;
		 if `ps`j'' == 2 {;
		 matrix `ma'[`j',1]=.;
		 matrix `ma'[`j',2]=.;
		 matrix `ma'[`j',3]=.;
	   ...
	   matrix colnames `ma' = "Initial price" "Subsidy"   "Final price" ;
	   /*
	  ...
	  capture {;
	    ...
	    lines(COL_NAMES 14 `nitems' 2 LAST_ROW 13)  newappend save(`xfil') sheet(General Info);
		 ...
		 if ( `nps2' != 0 ) { ;
		   forvalues i=1/`nitems' {;
		local ittit = ""+`slitems'[`i'];
		if `ps`i''==2 & `oinf' == 1 {;
		tempname AA;
		pschdes2 i_psch`i' f_psch`i' , item(`ittit'); 
		
		if `e(str1)'==3 local not1 = " - Initial price: IBT structure until block `e(bct1)' and VDT for suceedent blocks." ;
		if `e(str2)'==3 local not2 = " - Final   price: IBT structure until block `e(bct2)' and VDT for suceedent blocks." ;
        matrix `AA' = e(maps);
		mk_xtab_m2 `exp_pc' ,   matn(`AA') dec(2) xfil(`xfil') xshe(Price schedule_`ittit') xtit("The price schedule of the item: `ittit'") xlan(`lan') dste(-1)
		note( "`not1'" , "`not2'") 
		;
        };
		if `oinf' == 2 pschdes `ipsh1_`i'';
		 forvalues s = 1/`nscen' {;
		  if `oinf' == 2 pschdes `fpsh`s'_`i'';
		};
...
subjob24 `vlist',   hs(`hsize') hgroup(`hgroup') lan(`lan')  xrnames(`slist')  aggr(`taggregate') pcexp(`exp_pc') ipsch(`ipsch')  unit(`unit') gvqnt(`gvqnt') ;


subjob44 `vlist',   hs(`hsize') hgroup(`hgroup') lan(`lan')  xrnames(`slist')  aggr(`taggregate') pcexp(`exp_pc') ipsch(`ipsch') fpsch(`fpsch1') elas(`elas1') wappr(`wappr')  unit(`unit') ;
...
set tracedepth 10;
subjob`tjob`i'' `vlist',   hs(`hsize') hgroup(`hgroup') lan(`lan')  xrnames(`slist')  aggr(`taggregate') pcexp(`exp_pc') ipsch(`ipsch')  unit(`unit') gvqnt(`gvqnt') ;
tempname  mat`tjob`i'' ;
matrix `mat`tjob`i'''= e(est);
tabtit `tjob`i'' `lan'; local tabtit = "`r(tabtit)'";
distable `mat`tjob`i''', dec(`dec`tjob`i''') atit(`langr') head1(`r(tabtit)')  head2(`head2') ;
...
if (`tjob`i'' == 42 & `_tab_42' == 0 & `gvimp'==1) {;
*set trace on;
subjob`tjob`i'' `vlist',   hs(`hsize') hgroup(`hgroup') lan(`lan')  xrnames(`slist')  aggr(`taggregate') pcexp(`exp_pc') ipsch(`ipsch') fpsch(`fpsch`s'') wappr(`wappr') gvimp(`gvimp');
local suk`tjob`i'' = 1;
tempname my`tjob`i''_`s';
...
set more off;
set tracedepth 2;
subjob`tjob`i'' `vlist',   hs(`hsize') hgroup(`hgroup') lan(`lan')  xrnames(`slist')  aggr(`taggregate') pcexp(`exp_pc') ipsch(`ipsch') fpsch(`fpsch`s'') elas(`elas`s'') wappr(`wappr')  unit(`unit') gvqnt(`gvqnt') ;
tempname  mat`tjob`i'' ;
matrix `mat`tjob`i'''= e(est);
tabtit `tjob`i'' `lan'; 
local tabtit = "`r(tabtit)'"+"`scena'";
...
subjob`tjob`i'' `vlist',   hs(`hsize') aehs(`aehs') hgroup(`hgroup') lan(`lan')  xrnames(`slist')  aggr(`taggregate') pcexp(`exp_pc') ipsch(`ipsch') fpsch(`fpsch`s'') pline(`pline') wappr(`wappr');
tempname  mat`tjob`i'' ;
matrix `mat`tjob`i'''= e(est);
tabtit `tjob`i'' `lan'; 
local tabtit = "`r(tabtit)'"+"`scena'";
...
/*
if (`_tab_24' == 0 &  `gvqnt'==1 ) {;
subjob24 `vlist',   hs(`hsize') hgroup(`hgroup') lan(`lan')  xrnames(`slist')  aggr(`taggregate') pcexp(`exp_pc') ipsch(`ipsch')  unit(`unit') gvqnt(`gvqnt') ;
};
if (`_tab_44' == 0 &  `gvqnt'==1 ) {;
subjob44 `vlist',   hs(`hsize') hgroup(`hgroup') lan(`lan')  xrnames(`slist')  aggr(`taggregate') pcexp(`exp_pc') ipsch(`ipsch') fpsch(`fpsch1') elas(`elas1') wappr(`wappr')  unit(`unit') ;
};
*/
...
qui replace  `sum_res_I'  = round(el(`my`code'',`pos_last_row',`cola')/1000000, 0.01) in 4 ; 
*cap matrix drop `my`code'';
...
local pos_last_row rowsof(`my`code'');
local cola = colsof(`my`code'');
qui replace  `sum_res_I'  = round(el(`my`code'',`pos_last_row',`cola'), 0.01)        in 7 ; 
*cap matrix drop `my`code'';
...
local cola = colsof(`my`code'_`s'');
local pos_last_row rowsof(`my`code'_`s''); 
qui replace  `sum_res_S'  =   round(`sum_res_I'[7]+el(`my`code'_`s'',`pos_last_row',`cola'), 0.01) in 7 ;
...
local cola = colsof(`my`code'_`s'');
local pos_last_row rowsof(`my`code'_`s''); 
qui replace  `sum_res_S'  =   round(`sum_res_I'[4]-el(`my`code'_`s'',`pos_last_row',`cola')/1000000, 0.01) in 4 ; 
