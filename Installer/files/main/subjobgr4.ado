...
local str_`z'  =  `.`ipsch'.str'; 
local bcut_`z'  =  `.`ipsch'.bcut'; 
local sfee_`z'  =  `.`ipsch'.sfee'; 
...
cap drop `str1_`z''; 
local     bcut1_`z' = `bcut_`z'' + 1;

                     qui gen     `str1_`z'' = 1 in           1/`n_`z'';
if `str_`z'' == 2 	 qui replace `str1_`z'' = 2 in           1/`n_`z'';
if `str_`z'' == 3 	 qui replace `str1_`z'' = 2 in `bcut1_`z''/`n_`z'';
...
forvalues i = 2/`n_`z'' {;
local j  = `i'-1;
local se`i'_`z' = `se`j'_`z'' + (`q_`z''[`i']-`q_`z''[`j'])*`p1_`z''[`i'];
};


local ex1_`z'= `q_`z''[1]*`p1_`z''[1];
local ex0_`z'= 0;
forvalues i = 2/`n_`z'' {;
local j  = `i'-1;
local ex`i'_`z'= (`q_`z''[`i']-`q_`z''[`j'])*`p1_`z''[`i'];
};
...
qui replace `bexp_`z'' = max(0,`bexp_`z''-`sfee_`z'') ;
...
forvalues i = 2/`n1_`z'' {;
local j=`i'-1;
 qui replace `class_`z'' = `i'  if `bexp_`z''>(`q_`z''[`j']*`p1_`z''[`i'])  ; 
};
};
if `n_`z'' >= 2 qui replace `class_`z'' = `n_`z''         if (`bexp_`z''>`q_`z''[`n_`z''-1]*`p1_`z''[`n_`z''])  ;
};
...
if  `str_`z''==3      {;
if `n1_`z'' > 1 {;
forvalues i = 2/`bcut_`z'' {;
local j=`i'-1;
 qui replace `class_`z'' = `i'  if (`bexp_`z''>`se`j'_`z'') ;
};
};
local bc11 = `bcut_`z''-1;
qui replace `class_`z'' = `bcut_`z''         if (`bexp_`z''>`se`bc11'_`z'')  ;

local bcut11= `bcut_`z''+1;
if `n1_`z'' > 1 {;
forvalues i = `bcut11'/`n1_`z'' {;
local j=`i'-1;
 qui replace `class_`z'' = `i'  if `bexp_`z''>(`q_`z''[`j']*`p1_`z''[`i'])  ; 
};
};
...
/********************/
/*
Since the proportional increase in prices is the same in all blocks
the results of the impact are OK for the str2 and st3 structures. 
*/
...
local k = `i'-1;
forvalues j = 1/`k' {;
qui replace _wvar_`nva' = `basev`i'_`z''   - (`bexp_`z''-`se`j'_`z''     )*(`pos'/100)                               if (`class_`z''==`i');
};

qui replace _wvar_`nva' = `base`n_`z''_`z'' - (`bexp_`z''-`se`n1_`z''_`z'')*(`pos'/100)                           if `class_`z''==`n_`z''; 
