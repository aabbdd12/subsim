...
forvalues i = 1/`n' {;
forvalues h = 1/`n' {;
local sb`i'_`h' = `sub1_`h''[`i']/`p1_`h''[`i'];
};
};
...
if `n' >= 2 qui replace `class' = `n'         if (`bexp_`z''>`se`n1'')  ;
};
...
local bc1 = `bcut'-1;
qui replace `class' = `bcut'         if (`bexp_`z''>`se`bc1'')  ;
...
forvalues i = 1/`n' {;
forvalues h = 1/`n' {;
local lp2_`h' = `p1_`h''[`i']+`sub1_`h''[`i']*(`pos'/100);
local dp`i'_`h' = (`lp2_`h''/`p1_`h''[`i']-1) ;
};
};
