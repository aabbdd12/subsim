...
tempvar wvar ;
qui gen `wvar' = 0 ;
qui replace `wvar' = -`bexp'*(`p2_1'[1]/`p1_1'[1]-1)                                                         if `class'==1; 
if `nn1' > 1 {;
forvalues i = 2/`nn1' {;
local j=`i'-1;
 qui replace `wvar' = `basev`i'_`i'' - (`bexp'-`se`j'_`i'')*(`p2_`i''[`i']/`p1_`i''[`i']-1)                             if (`class'==`i') ;
};
};
if `nn1'>=1  qui replace  `wvar' = `basev`nn'_`nn'' - (`bexp'-`se`nn1'_`nn'')*(`p2_`nn''[`nn']/`p1_`nn''[`nn'] - 1)           if (`class' == `nn' )  ;
