...
if  "``name'`i''"=="." local `name'`i' = "" ;
  ...
  if (`gvimp'==1)     file write myfile `".asubsim_dlg.main.chk_gvimp.seton "' _n; 
  if (`gvqnt'==1)     file write myfile `".asubsim_dlg.main.chk_gvqnt.seton "' _n; 
   ...
   file write myfile `".asubsim_dlg.tb_options.ed_aggr.setvalue "`taggregate'""' _n;
   };
   
    if ("`gaggregate'"~="") {;
   file write myfile `".asubsim_dlg.gr_options.gck_order.seton"' _n;
   file write myfile `".asubsim_dlg.gr_options.ged_aggr.setvalue "`gaggregate'""' _n;
   };
   ...
   if (`summary'==1)  file write myfile `".asubsim_dlg.tb_options.chk_sum.seton"' _n;
   if ("`gjobs'"~="") {;
...
local bun  =  `.`ipp`i''.bun' ; 
local str  =  `.`ipp`i''.str' ; 
local bcut  =  `.`ipp`i''.bcut' ; 
local sfee  =  `.`ipp`i''.sfee' ; 

local tcmd `tcmd' nblock(`n') bun(`bun') str(`str') bcut(`bcut') sfee(`sfee')  ;
local n1=`n'-1;
forvalues j = 1/`n1' {;
...
file write myfile `"global r_i_`i' = 1"'  _n;
...
local bun  =  `.`fpp`i''.bun' ; 
local str  =  `.`fpp`i''.str' ; 
local bcut  =  `.`fpp`i''.bcut' ; 
local sfee  =  `.`fpp`i''.sfee' ;
local tcmd `tcmd' nblock(`n') bun(`bun') str(`str') bcut(`bcut') sfee(`sfee')  ;
local n1=`n'-1;
forvalues j = 1/`n1' {;
...
file write myfile `"`tcmd'"'  _n;
file write myfile `"global r_f_`i' = 1"'  _n;
...
if ("`ps`i''" == "2") {;
	...
	file write myfile `" if ("$"';
	file write myfile `"prg_pointer"== "ps_i_`i'") "';
	file write myfile `" & ($"';
	file write myfile `"r_i_`i'==1)"';
	file write myfile "{" _n;
   file write myfile `".pschset_i_`i'_dlg.main.cb_ini.setvalue     `.i_psch`i'.oin[1].oinf'"'          _n;
   file write myfile `".pschset_i_`i'_dlg.main.cb_bracs.setvalue   `.i_psch`i'.nblock'"'                _n;
   file write myfile `".pschset_i_`i'_dlg.main.ed_bracs.setvalue   `.i_psch`i'.nblock'"'                _n;
   file write myfile `".pschset_i_`i'_dlg.main.cb_bun.setvalue     `.i_psch`i'.bun'"'                   _n;
   file write myfile `".pschset_i_`i'_dlg.main.cb_str.setvalue     `.i_psch`i'.str'"'                   _n;
   file write myfile `".pschset_i_`i'_dlg.main.ed_cut.setvalue     `.i_psch`i'.bcut'"'                   _n;
   file write myfile `".pschset_i_`i'_dlg.main.ed_sfee.setvalue     `.i_psch`i'.sfee'"'                   _n;
   
   
  if  ( `.i_psch`i'.oin[1].oinf' == 1 ) {;  
   local nblk = `.i_psch`i'.nblock';
   
 forvalues j=1/`nblk' {;
 file write myfile `".pschset_i_`i'_dlg.main.en_mxb`j'.setvalue    " `.i_psch`i'.blk[`j'].max' ""'  _n;  
 file write myfile `".pschset_i_`i'_dlg.main.en_tarif`j'.setvalue  " `.i_psch`i'.blk[`j'].price' ""'  _n; 
 file write myfile `".pschset_i_`i'_dlg.main.en_sub`j'.setvalue    " `.i_psch`i'.blk[`j'].subside' ""'  _n; 
  };
  }; 
  
 if  ( `.i_psch`i'.oin[1].oinf' == 2 ) {;
 file write myfile `".pschset_i_`i'_dlg.main.var_mxb.setvalue     `.i_psch`i'.oin[1].vmax'  "'  _n; 
 file write myfile `".pschset_i_`i'_dlg.main.var_tr.setvalue      `.i_psch`i'.oin[1].vprice' "'   _n; 
 file write myfile `".pschset_i_`i'_dlg.main.var_sub.setvalue     `.i_psch`i'.oin[1].vsub' "'  _n; 
 };
  file write myfile `"global r_i_`i' = 0"'  _n; 
file write myfile "}" _n;

file write myfile `" if ("$"';
	file write myfile `"prg_pointer"== "ps_f_`i'") "';
	file write myfile `" & ($"';
	file write myfile `"r_f_`i'==1)"';
	file write myfile "{" _n;
   file write myfile `".pschset_f_`i'_dlg.main.cb_ini.setvalue     `.f_psch`i'.oin[1].oinf'"'          _n;
   file write myfile `".pschset_f_`i'_dlg.main.cb_bracs.setvalue   `.f_psch`i'.nblock'"'                _n;
   file write myfile `".pschset_f_`i'_dlg.main.ed_bracs.setvalue   `.f_psch`i'.nblock'"'                _n;
   file write myfile `".pschset_f_`i'_dlg.main.cb_bun.setvalue     `.f_psch`i'.bun'"'                   _n;
  file write myfile `".pschset_f_`i'_dlg.main.cb_str.setvalue     `.f_psch`i'.str'"'                   _n;
  file write myfile `".pschset_f_`i'_dlg.main.ed_cut.setvalue     `.f_psch`i'.bcut'"'                   _n;
  file write myfile `".pschset_f_`i'_dlg.main.ed_sfee.setvalue     `.f_psch`i'.sfee'"'                   _n;
   
  if  ( `.f_psch`i'.oin[1].oinf' == 1 ) {;  
   local nblk = `.f_psch`i'.nblock';
   
 forvalues j=1/`nblk' {;
 file write myfile `".pschset_f_`i'_dlg.main.en_mxb`j'.setvalue    " `.f_psch`i'.blk[`j'].max' ""'  _n;  
 file write myfile `".pschset_f_`i'_dlg.main.en_tarif`j'.setvalue  " `.f_psch`i'.blk[`j'].price' ""'  _n; 
 file write myfile `".pschset_f_`i'_dlg.main.en_sub`j'.setvalue    " `.f_psch`i'.blk[`j'].subside' ""'  _n; 
  };
  }; 
  
 if  ( `.f_psch`i'.oin[1].oinf' == 2 ) {;
 file write myfile `".pschset_f_`i'_dlg.main.var_mxb.setvalue     `.f_psch`i'.oin[1].vmax'  "'  _n; 
 file write myfile `".pschset_f_`i'_dlg.main.var_tr.setvalue      `.f_psch`i'.oin[1].vprice' "'   _n; 
 file write myfile `".pschset_f_`i'_dlg.main.var_sub.setvalue     `.f_psch`i'.oin[1].vsub' "'  _n; 
 };
 
  file write myfile `"global r_f_`i' = 0"'  _n; 
file write myfile "}" _n;
};
