...
if ("`ps`i''" == "2" & "`tmp'"=="")  {;
  ...
  tokenize `inisave' ,  parse(".");
  local nameps="i_psch`i'";
  local dof = trim("`1'");
  local fname "`dof'-`nameps'.pco" ;
 local fname2 $tempprj ;
 local fname2 `fname2'-i_psch`i'.pco ;
 if ("`fname2'"!="`fname'") {;
 copy `fname2' `fname', replace;
 };
...
if ("`ps`i''" == "2" & "`tmp'"=="")  {;
  tokenize `inisave' ,  parse(".");
  local nameps="f_psch`i'";
  local dof = trim("`1'");
  local fname "`dof'-`nameps'.pco" ;
 local fname2 $tempprj ;
 local fname2 `fname2'-f_psch`i'.pco ;
 if ("`fname2'"!="`fname'") {;
 copy `fname2' `fname', replace;
 };
