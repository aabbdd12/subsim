...
capture program drop itjobstat;
program define itjobstat, eclass;
version 9.2;
...
forvalues k = 1/$indica {;
it_basicstats ``k'' ,  hhid(`hhid') hsize(`_ths') hgroup(`hgroup') stat(`stat') denom(`denom') income(`income');
...
it_basicstats `total' ,  hhid(`hhid') hsize(`_ths')  hgroup(`hgroup') stat(`stat') denom(`denom') income(`income');
local kk1 = $indica+1;
forvalues i=1/`indig1' {;
...
if "`stat'"=="inc_pc"  local tab_tit = "Total incomes per capita";
...
if "`stat'"=="inc_pc"  local tab_tit_fr = "Revenues per capita";
